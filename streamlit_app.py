...
TEXTOS = {
    "Português": {
        ...
        "avaliacao_1": "1 · Precisa melhorar",
        "avaliacao_2": "2 · Bom",
        "avaliacao_3": "3 · Excelente",
    ...
    "English": {
        ...
        "avaliacao_1": "1 · Needs improvement",
        "avaliacao_2": "2 · Good",
        "avaliacao_3": "3 · Excellent",
    ...
    "Español": {
        ...
        "avaliacao_1": "1 · Necesita mejorar",
        "avaliacao_2": "2 · Bueno",
        "avaliacao_3": "3 · Excelente",
...
def resposta_final(resposta):
    ...
    return tem_encaminhamento and tem_justificacao
...
if pergunta:
    ...
    with st.chat_message("assistant"):
        ...
        if any(palavra in resposta_bot.lower() for palavra in ["emergência", "emergency", "emergencia", "112"]):
...
if st.session_state.mostrar_avaliacao:
    ...
    if not st.session_state.avaliacao_enviada:
        ...
        with col1:
            if st.button(t["avaliacao_1"], key=f"avaliacao_1_{st.session_state.session_id}"):
                st.session_state.nota_avaliacao = 1
                st.session_state.avaliacao_enviada = True
                st.rerun()

        with col2:
            if st.button(t["avaliacao_2"], key=f"avaliacao_2_{st.session_state.session_id}"):
                st.session_state.nota_avaliacao = 2
                st.session_state.avaliacao_enviada = True
                st.rerun()

        with col3:
            if st.button(t["avaliacao_3"], key=f"avaliacao_3_{st.session_state.session_id}"):
