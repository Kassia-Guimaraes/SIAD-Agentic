...
Formato em inglês:
- referral: [112 - INEM / Emergency Service (SU) / Health Center (CSP) / Self-care (Home treatment + reassessment)]
...
def guardar_avaliacao(session_id, nota, pergunta_utente, resposta_sistema):
...
def formatar_historico_sessao(session_id, limite=6):
    """
    Recupera as últimas interações da sessão atual e transforma em texto
    para enviar ao GPT-4o como memória da conversa.
    """

    try:
        mensagens = obter_mensagens_sessao(session_id)
    except Exception:
    ...
    # Usa apenas as últimas interações para não deixar o prompt demasiado grande
    mensagens_recentes = mensagens[-limite:]

    historico = []

    for pergunta, resposta in mensagens_recentes:
        historico.append(f"Utente: {pergunta}")
...
def perguntar_sns24(sintomas, prompt_template, base_dados, session_id):
    ...
    historico_conversa = formatar_historico_sessao(session_id)
...
if __name__ == "__main__":
    ...
    session_id_terminal = str(uuid.uuid4())
    ...
    while True:
        ...
        resposta = perguntar_sns24(sintomas, prompt, base_dados, session_id=session_id_terminal)
